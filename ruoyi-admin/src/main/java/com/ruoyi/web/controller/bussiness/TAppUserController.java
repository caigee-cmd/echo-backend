...
//        changeUserAppParentIds(user);
    ...
    @PreAuthorize("@ss.hasPermi('bussiness:user:updateLimitationStatus')")
    @Log(title = "修改用户限制状态状态", businessType = BusinessType.UPDATE)
    @PutMapping("/updateLimitationStatus")
    public AjaxResult updateLimitationStatus (@RequestBody TAppUser tAppUser){
        return toAjax(tAppUserService.updateLimitationStatus(tAppUser));
    }
